			...
			<div class="inner-wrapper">
				...
				<section role="main" class="content-body">
					<header class="page-header">
						...
						<div class="right-wrapper pull-right">
							<ol class="breadcrumbs">
								...
								<li><span>Manage Location</span></li>
								<li><span>Update City</span></li>
							</ol>
					...
					<div class="row">
						<div class="col-md-15">
						<?php
							$res = mysql_query("select * from tbl_city where city_id='".$_REQUEST['update']."'");
						...
						?>
							<form id="form1" class="form-horizontal" method="post">
								<section class="panel">
										...
										<div class="form-group">
											...
											<div class="col-sm-9">
												<select id="state" class="form-control" required="" name="state">
													<?php
														$result = mysql_query("select * from tbl_state");
														while($row = mysql_fetch_array($result))
														{
													?>
														<option value = "<?php echo $row["state_id"]; ?>" <?php if($r['state_id']==$row['state_id']) {?> selected="selected" <?php } ?> ><?php echo $row["state_name"]; ?></option>
													<?php													
														}
													?>
												</select>
									...
									<div class="panel-body">
										<div class="form-group">
											<label class="col-sm-3 control-label">Update City<span class="required">*</span></label>
											<div class="col-sm-9">
												<input type="text" name="addcity" class="form-control" placeholder="eg.: Surat" data-bvalidator="alpha,required" value="<?php echo $r["city_name"]; ?>"/>
											</div>
										</div>								
									</div>
									<footer class="panel-footer">
										<div class="row">
											<div class="col-sm-9 col-sm-offset-3">
												<button class="mb-xs mt-xs mr-xs btn btn-warning" name="btnaddcity" type="submit">Update City</button>											
											</div>
										</div>
									</footer>
								</section>
						...
						{
						 mysql_query("update tbl_city set city_name='".$_REQUEST['addcity']."',state_id ='".$_REQUEST['state']."' where city_id='".$_REQUEST['update']."'");
						 echo "<script>window.location='view_city.php';</script>";
						}
						?>
						<!-- col-md-6 -->
